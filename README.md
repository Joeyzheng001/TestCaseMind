...
这套注册表在一致性引擎、方法论选择、真实文献猎手、章节写作四个模块中共享，确保**方法的定义、使用场景、操作步骤在所有章节中保持一致**。
...
同时计算 7 维质量评分（文献类型、字段完整度、基础性、方法相关性、长度、时效性、噪音），低分卡片在引用推荐时被降权。
...
| 06 真实文献猎手 | 联网寻找 3-5 篇真实存在、可验证、高相关的参考文献 | OpenAlex 可验证来源 + 相关性排序 + 不生成伪引用 |
