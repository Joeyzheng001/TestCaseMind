    ...
    for rec in records:
        conn.execute(
            ...
            (
                ...
                rec.get("category", ""), json.dumps(rec.get("phase", [])),
                json.dumps(rec.get("disciplines", [])), json.dumps(rec.get("domains", [])),
                json.dumps(rec.get("applicable_sections", [])), json.dumps(rec.get("aliases", [])),
                json.dumps(rec.get("pairs_with", [])), json.dumps(rec.get("requires", [])),
                json.dumps(rec.get("conflicts_with", [])), rec.get("difficulty", ""),
                json.dumps(rec.get("data_type", [])), json.dumps(rec.get("outputs", [])),
                json.dumps(rec.get("risk_tags", [])), rec.get("source_type", ""),
