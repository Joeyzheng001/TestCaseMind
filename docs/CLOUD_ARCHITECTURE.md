...
| 资源 | 规格 | 月费 |
|------|------|------|
| 轻量 ECS | 1核2G 3Mbps | ¥60-100 |
| RDS PostgreSQL | 1核1G 20G SSD | ¥100-220 |
| OSS 对象存储 | 10G + 流量 | ¥10-20 |
| **合计** | | **¥170-340/月** |

优势：RDS PG 托管最成熟，自动备份扩容免运维；国内文档最全，踩坑可搜。

### 12.2 备选：腾讯云

轻量服务器三年折扣大，PG 支持不如阿里云但够用。适合预算敏感场景。

### 12.3 海外用户场景

| 方案 | 工具 | 月费 |
|------|------|------|
| 一站式部署 | Railway / Fly.io | $0-25 |
| 免费 PG | Supabase（500MB 免费层） | $0 |

总成本可从月 ¥300+ 降到月 $0-25。

### 12.4 容量估算（基础配置 ¥200-400/月）

| 资源 | 瓶颈点 |
|------|--------|
| ECS（1核2G） | 仅做转发 + 等 LLM 返回，自身消耗极低 |
| RDS（1核1G） | 知识库 pgvector 检索最吃内存，单次 50-200ms，并发 20-30 开始排队 |
| OSS | 更新包直传，不占 ECS |
...
```
当前（¥200-400/月）          量上来后（¥600-800/月）       大规模（¥1500+/月）
─────────────────          ──────────────────         ──────────────────
ECS  1核2G                 ECS  2核4G                 ECS  4核8G ×2
RDS  1核1G 20G             RDS  2核4G 50G             RDS  4核8G 100G
OSS  10G                   OSS  50G + CDN             OSS  100G + CDN
—                          Redis 缓存热查询             知识库独立部署
