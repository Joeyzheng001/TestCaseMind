...
│   ├── 01_风控表.md      ← 按模块拆分，单次检索不超过相关模块
│   └── ...
└── design/
    ├── 00_因子索引.md    ← 因子设计文档索引
    └── 持仓数量因子.md   ← 每个因子的参数值域+元因子取值逻辑
```
...
```json
{
  "meta": {
    "total": 46,
    "by_source": { "REQ": 38, "KB": 4, "RISK": 4 }
  },
  "testpoints": [
    {
      "testpoint_id": "TP-001",
      "functional_module": "基本公式验证",
      "test_scenario": "正常计算：持仓数量=countif(持仓数量,'>0')",
      "source": "REQ",
      "source_ref": "需求第2节：持仓数量计算公式",
      "preconditions": "产品A持有证券STK_001持仓数量=1000，STK_002持仓数量=2000",
      "test_steps": "1. 准备测试数据\n2. 调用持仓数量计算接口\n3. 查询返回值",
      "expected_result": "返回 2，即 countif([1000, 2000], '>0') = 2",
      "priority": "P0",
      "remarks": ""
    },
    {
      ...
      "test_scenario": "投资类型传入非法枚举值时的处理",
      "source": "KB",
      "source_ref": "数据字典枚举值.md - 投资类型合法值：1/2/3/8/9/16",
      "preconditions": "持仓数据中存在投资类型=99（非法值）",
      "test_steps": "1. 插入投资类型=99的持仓记录\n2. 调用计算接口",
      "expected_result": "返回参数校验错误或忽略该条记录，不影响合法数据计算",
      "priority": "P1",
    ...
    },
    {
      ...
      "test_scenario": "多产品同时触发持仓数量计算时的结果准确性",
      "source": "RISK",
      "source_ref": "风险推断：需求未说明并发计算的隔离机制",
      "preconditions": "5个产品同时发起持仓数量计算请求",
      "test_steps": "1. 并发发送5个计算请求\n2. 等待所有请求返回\n3. 对比各产品计算结果",
      "expected_result": "每个产品返回各自正确的持仓数量，结果互不影响",
      "priority": "P2",
      "remarks": "需确认是否有产品级别的计算隔离"
    }
  ]
}
```
...
### 评审分: 72
### 测试点总数: 46
### REQ需求直出: 38 | KB知识库: 4 | RISK风险: 4
## ⚠ 风险项
### [performance] 未定义事中监控的实时性要求和并发上限
### [integration] 依赖 DWD_AST_PD_HLDP_INFO 表，未说明数据同步延迟处理
## 基本公式验证 (8条)
### 🔵[REQ][P0] 正常计算：持仓数量=countif(持仓数量,'>0')
### 🔵[REQ][P0] 边界处理：所有持仓数量为0时返回0
...
## 数据字典约束 (4条)
### 🟡[KB][P1] 投资类型传入非法枚举值时的处理
...
## 并发与数据一致性 (4条)
### 🔴[RISK][P2] 多产品同时触发持仓数量计算时的结果准确性
...
```
...
| 项目 | 内容 |
|------|------|
| 需求文档 | ARM-RULE-UC00001_持仓数量类因子规格文档-2.6.6.md |
| 需求质量 | 🟡 良（72/100）|
| 测试覆盖 | 🟢 完整 |
...
- ✅ 共 12 条 P0 核心测试点，上线前必须全部通过。
- ✅ 共生成 46 条测试用例，可直接导入测试管理工具执行。
